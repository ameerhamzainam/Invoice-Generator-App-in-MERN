...
function Center() {
    ...
    const filter = ['paid', 'Unpaid'];
    const [isDropdown, setIsDropdown] = useState(false);
    const [openCreateInvoice, setOpenCreateInvoice] = useState(false);

    useEffect(() => {
        axios
            .get('http://localhost:5555/invoices')
            .then((response) => {
    ...
    const filterInvoices = () => {
        if (!filterValue) {
            ...
            const filtered = invoices.filter(
                (invoice) => invoice.invoiceStatus === filterValue
            );
    ...
    return (
        <div>
            <div className='dark:bg-[#1e2124] scrollbar-hide duration-300 min-h-screen bg-[#f8f8fb] py-[34px] px-2 md:px-8 lg:px-12 lg:py-[72px]  '>
                <motion.div
                    ...
                    <div className=' min-w-full max-h-[64px] flex items-center justify-between'>
                        ...
                        <div className=' flex  max-h-full  items-center '>
                            ...
                            {isDropdown &&
                                ...
                                } className='  w-40 bg-white dark:bg-[#36393e] dark:text-white flex px-6 py-4 flex-col  top-[160px] lg:top-[120px]  absolute  shadow-2xl rounded-xl space-y-2    '>
                                    {filter.map((item, i) => (
                                        <div key={i} onClick={() => { item === filterValue ? setFilterValue('') : setFilterValue(item) }} className=' items-center cursor-pointer flex space-x-2 '>
                                            <input value={item} checked={filterValue === item ? true : false} type='checkbox' className=' accent-[#7289da] hover:accent-[#7289da] ' />
                                            <p>
                                                {item}
                                            </p>
                                        </div>
                            ...
                            <button onClick={() => setOpenCreateInvoice(true)} className=' hover:opacity-80 ml-4 md:ml-10 flex items-center py-2 px-2 md:space-x-3 space-x-2 bg-[#7289da] rounded-full'>
                                <img src={plus} alt="" />
